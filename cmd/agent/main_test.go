...
// The startup card must survive terminals that disagree with runewidth about
// glyph widths (a double-width ✦ used to push the first row past the right
// border). Guard the invariant: the brand line sits ABOVE the box, and every
// box line is pure ASCII apart from the border glyphs, all the same width.
func TestBannerBoxASCIIAndAligned(t *testing.T) {
	lines := bannerLines("ipsupport-code", "v0.24.1", "local", "qwen2.5-7b-instruct", "/home/x/myproject", 24_000, lipgloss.Color("205"))
	if len(lines) < 3 {
		t.Fatalf("banner too short: %q", lines)
	}
	head := stripAnsi(lines[0])
	if !strings.Contains(head, "✦ ipsupport-code") || !strings.Contains(head, "v0.24.1") {
		t.Fatalf("brand line missing from head: %q", head)
	}
	borders := "╭╮╰╯─│"
	width := -1
	boxLines := 0
	for _, ln := range lines[1:] {
		s := stripAnsi(ln)
		if s == "" || !strings.ContainsAny(s, borders) {
			continue // tip/keys lines below the box
		}
		boxLines++
		inner := s
		for _, r := range inner {
			if !strings.ContainsRune(borders, r) && r > 127 {
				t.Fatalf("non-ASCII rune %q inside box line %q — width-ambiguous on some terminals", r, s)
			}
		}
		w := len([]rune(s)) // border glyphs + ASCII: rune count == display width
		if width == -1 {
			width = w
		} else if w != width {
			t.Fatalf("box misaligned: line %q is %d wide, want %d", s, w, width)
		}
	}
	if boxLines < 4 { // top border + model + cwd + ctx + bottom border
		t.Fatalf("expected a box under the brand line, got %d box lines: %q", boxLines, lines)
	}
}
